...
generateAndSaveOtp(user.id).then((otp) => {
            logger.info(`OTP generated and saved for user ${user.id}: ${otp}`);
            sendVerificationEmail(user.email, otp).then(() => {
                logger.info(`Verification email sent to ${user.email}`);
            }).catch((error) => {
                logger.error(`Error sending verification email: ${error}`);
            });
        }).catch((error) => {
            logger.error(`Error generating or saving OTP: ${error}`);
        });
    return {
        user: {
            id: user.id,
...
export const verifyOtp = async (userId: string, otp: string, purpose: otpPurpose) => {
    try {
      ...
      logger.info(`Verifying OTP for user ${userId} with purpose ${otp} `);
